...
export default function ContactPage() {
  ...
  const sendMessage = async () => {
    ...
    if (!trimmed) return;
    const newConversation = [...conversation, { role: 'user', message: trimmed }];
    ...
    try {
      ...
      if (data.escalate) {
        // auto fill subject and message for ticket form
        setFormData((f) => ({
          ...f,
          subject: trimmed.slice(0, 60),
          message: newConversation.map((m) => `${m.role === 'user' ? 'User' : 'Support'}: ${m.message}`).join('\n'),
        }));
        setPhase('form');
      }
    } catch (e) {
          ...
          <>
            <div className="bg-primary-light p-4 rounded-md h-96 overflow-y-auto mb-4 space-y-2">
              {conversation.map((msg, idx) => (
                <div
                  key={idx}
                  className={`p-2 rounded-md max-w-xs ${msg.role === 'user' ? 'bg-accent-light text-primary self-end ml-auto' : 'bg-accent-dark text-white self-start'}`}
                >
            ...
            <div className="flex space-x-2">
              ...
              <button
                onClick={sendMessage}
                className="bg-accent px-4 py-2 rounded-md text-white hover:bg-accent-dark"
              >
                Send
              </button>
            </div>
        ...
        {phase === 'form' && (
          <div className="bg-primary-light p-6 rounded-md">
            ...
            <p className="text-sm mb-4">
              Please fill out the form below to create a ticket. Our team will get back to you via
              email. Do not include passwords or sensitive information.
            </p>
          ...
          <div className="bg-primary-light p-6 rounded-md text-center">
            ...
            {privateKey && (
              <p className="mb-4 text-xs text-gray-400">
                Private Info Key: {privateKey} (included in the ticket email template)
              </p>
            )}
            <p className="mb-4">
              Our team will respond as soon as possible. If you do not hear back within a
              reasonable time or wish to request a copy of your transcript, you may
              email us using the button below. This will open Gmail compose with a prefilled
              template. Do not change the message contents.
            </p>
            <a
